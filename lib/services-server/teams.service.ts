...
export class TeamsService {
   ...
   */
  static async createTeam(data: { name: string; userId: string; instagram?: string; league?: string }) {
    // Validaciones
    ...
    const sanitizedLeague = data.league?.trim() || undefined;
    ...
    if (sanitizedLeague && sanitizedLeague.length > 100) {
      throw new ValidationError('La liga no puede exceder 100 caracteres');
    }
